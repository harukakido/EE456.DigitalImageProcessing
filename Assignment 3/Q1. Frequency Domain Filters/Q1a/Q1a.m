...
%15x15 average filter to filter image in frequency domain
sizeofavgmask = 15
half_sizeofavgmask = floor(sizeofavgmask/2);
%avgfilterA = fspecial('average', sizeofavgmask);
%B = imfilter(A, avgfilterA);
mask = zeros(r, c);
%dimensions of white-centered box in filter set to 1
mask(r/2 - half_sizeofavgmask:r/2 + half_sizeofavgmask, c/2 - half_sizeofavgmask:c/2 + half_sizeofavgmask) = 1; 
subplot(1, 6, 3);
...
subplot(1, 6, 6);
imshow(log(1 + abs(filtered_img_spatial)), []); %show filtered image in spatial domain
